...
trait ZipkinSpanGenerator { self: App =>
  ...
  val maxSpanDepth = flag("maxSpanDepth", 7, "Max depth of generated traces")
  val maxSpans = flag("maxSpans", 10, "Max number of span names (equivalent to services)")

  def generateTraces(store: Seq[Span] => Future[Unit]): Future[Unit] = {
    val traceGen = new TraceGen(genTraces(), maxSpans(), maxSpanDepth())
    Future.join(traceGen() map { trace =>
      store(trace.spans.map(thriftSpanToSpan(_).toSpan))
    })
  }
}
